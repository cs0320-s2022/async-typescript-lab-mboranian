...
const sunMenu = document.querySelector('#sun') as  HTMLSelectElement
const moonMenu = document.querySelector('#moon') as HTMLSelectElement
const risingMenu = document.querySelector('#rising') as HTMLSelectElement
// Here, when the value of sun is changed, we will call the method postAndUpdate.
// @ts-ignore
sunMenu.addEventListener("change",postAndUpdate)
moonMenu.addEventListener("change",postAndUpdate)
risingMenu.addEventListener("change",postAndUpdate)
...
function postAndUpdate(): void {
  ...
  fetch('http://localhost:4567/results', {
    ...
    body: JSON.stringify(postParameters),
      ...
      .then((responseObject : Response) => responseObject.json())
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.forEach(match => {
    suggestion.innerHTML += `<li tabindex="0">${match}</li>`
  })
...
document.addEventListener('keyup',async (event : KeyboardEvent) => {
  if (event.key == "d") {
    await updateValues("Capricorn", "Pisces", "Cancer")
    postAndUpdate()
  }
})
...
async function updateValues(sunval: string, moonval: string, risingval: string): Promise<void>{
  ...
  sunMenu.value = sunval;
  moonMenu.value = moonval;
  risingMenu.value = risingval;
}
